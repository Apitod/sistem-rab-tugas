...
@section('header_title', 'Kwitansi — ' . $proposal->rab_number)
...
<div class="max-w-3xl space-y-4">
    ...
    @endif
    @if(session('info'))
    <div class="bg-blue-100 border border-blue-300 text-blue-800 px-4 py-3 rounded-lg text-sm no-print">
    ...
    <div class="flex items-center gap-3 no-print">
        <a href="{{ route('tu.kwitansi.index') }}" class="text-secondary hover:underline text-sm">
            <i class="fa-solid fa-arrow-left mr-1"></i> Kembali ke Daftar
        </a>
        <div class="ml-auto flex gap-2">
            {{-- Kirim ke Pengusul --}}
            <form method="POST" action="{{ route('tu.kwitansi.kirim', $proposal->id) }}" id="form-kirim">
                @csrf
                <button type="submit"
                        onclick="return confirm('Kirim notifikasi kwitansi ke {{ addslashes($proposal->user->name ?? 'pengusul') }}?')"
                    ...
                    <i class="fa-solid fa-paper-plane"></i> Kirim ke Pengusul
                </button>
            </form>
    ...
    <div id="kwitansi-doc" class="bg-white rounded-xl shadow-lg p-8 print-area relative overflow-hidden">
        ...
        <div class="border-b-4 border-double border-secondary pb-4 mb-6">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0 w-20 h-20">
                    {{-- Gunakan logo jika tersedia, fallback ke icon --}}
                ...
                {{-- Sisi kanan kosong untuk simetri --}}
        ...
        <div class="mb-8 bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3">
            ...
            <p class="text-sm text-gray-800 italic font-medium" id="terbilang-text">
    ...
    {{-- Tombol Kembali (tidak tercetak) --}}
    <div class="flex justify-between no-print">
        <a href="{{ route('tu.kwitansi.index') }}" class="text-secondary hover:underline text-sm">
            <i class="fa-solid fa-arrow-left mr-1"></i> Kembali
        </a>
        <div class="flex gap-2">
            <form method="POST" action="{{ route('tu.kwitansi.kirim', $proposal->id) }}">
                @csrf
                <button type="submit"
                        onclick="return confirm('Kirim notifikasi kwitansi ke pengusul?')"
                        class="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-emerald-700 transition-colors">
                    <i class="fa-solid fa-paper-plane"></i> Kirim ke Pengusul
                </button>
            </form>
